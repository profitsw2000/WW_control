...
2. Процедура обработки короткого нажатия кнопки. При выходе из состояния установки параметров таймера последним коротким нажатием кнопки, установленные
параметры записываются в eeprom. 
...
bsp_timer_mode_clear_segm:
	clr		set_segm
	cbr		state_flag,(1<<WWSM)
	call	Write_timer_parameters_to_eeprom		;записать параметры таймера в eeprom
	jmp		exit_bsp_proc
