...
14. Процедура увеличения счётчика времени, в течение которого часы находятся в режиме таймера, при этом таймер не запущен. Если значение счетчика
превысит максимальное значение (BT_MAX_TIME), то производится его обнуление и переход в режим часов.
15. Обнуление счетчика времени незапущенного таймера.
...
//считывание значения таймера
	ldi		XL,LOW(TIMER_MODE_COUNTER)				
	ldi		XH,HIGH(TIMER_MODE_COUNTER)
	ld		templ,X+
	ld		temph,X
//увеличить на 1
	ldi		temp_2,1
...
//проверить на достижение максимального значения счетчика незапущенного бокс. таймера
	...
	cpi		templ,LOW(BT_MAX_TIME)
	brlo	exit_increment_timer_mode_counter
//обнулить счетчик и перейти в режим часов	
	cbr		state_flag,(1<<WWVM)
